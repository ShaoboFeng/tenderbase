...
public interface GenericIndex<T extends IPersistent> extends IPersistent, IResource, Collection<T> { 
     ...
     * @return array of objects whose keys belong to the specified interval, ordered by key value
     */
    public ArrayList<T> getList(Key from, Key till);

     /**
     * Get objects whose key value belongs to the specified range.
     * Both from and till can be <code>null</code>.
     * If both are null, the method returns all objects from the index.
     * @param from low boundary. If <code>null</code> then low boundary is not specified.
     * Low boundary can be inclusive or exclusive. 
    ...
    /**
     * Get objects whose key value belongs to the specified range.
     * From and till can be <code>null</code>.
     * If both from and till are null, the method returns all objects from the index.
     ...
     * @return array of objects whose key belongs to the specified interval, ordered by key value
     */
    ...
    /**
     * Get objects whose key starts with a specified prefix,
     ...
     * @param prefix string key prefix
     * @return list of objects whose key starts with this prefix 
     */
     ...
     * @param word string whose prefixes are located in index
     ...
     * Locate all objects whose key is prefix of specified word.
     * @param word string whose prefixes are located in index
     * @return list of objects whose key is a prefix of specified word, ordered by key value
     */
